...
def _llm_judge(briefing: str, contact: str, company: str) -> dict:
    """LLM-as-judge : note la fiche sur 5 critères."""
...
def evaluate_langsmith(result: dict) -> dict:
    ...
    # Évaluation LLM-as-judge (tracée automatiquement via LangSmith)
    scores = _llm_judge(briefing, contact, company)
    overall = float(scores.get("overall", 0))

    # Récupérer le dernier run LangSmith du projet
    ...
    try:
        ...
        if api_key:
            ...
            if runs:
                run_url = f"https://smith.langchain.com/o/default/projects/p/{runs[0].session_id}/r/{runs[0].id}"
    except Exception:
        pass
