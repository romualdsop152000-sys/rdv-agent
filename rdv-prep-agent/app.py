...
with st.sidebar:
    ...
    st.caption({
        ...
        "human":
            "Après chaque génération, notez la fiche de 1 à 5 étoiles. "
            "Le score est sauvegardé en base.",
        "ragas":
            "RAGAS lance automatiquement un LLM juge après la génération. "
            "Métriques : faithfulness, answer_relevancy. "
            "Scores loggés dans MLflow.",
        "langsmith":
            "Un LLM juge (GPT-4o-mini) note la fiche sur 4 critères. "
            "Le run est tracé dans LangSmith. "
            "Nécessite LANGCHAIN_API_KEY dans .env.",
    }[eval_method])
    st.divider()
...
if submitted:
    ...
    with st.status("🔎 Collecte du contexte...", expanded=True) as status:
        ...
        st.write(f"✅ Contexte collecté en {context_duration}s"
                 + (f" · 📋 Notes CRM trouvées" if crm_found else ""))
    ...
    mlflow.end_run()  # sécurité : clôture tout run éventuellement actif
    ...
    # Pour l'éval humaine : on mémorise l'id en attente de notation
        ...
        with st.expander(label, expanded=is_latest):
            ...
            with col_meta:
                ...
                if entry["eval_score"] is not None:
                    ...
                    if entry["eval_method"] == "langsmith" and entry["eval_details"]:
                    ...
                    elif entry["eval_method"] == "ragas" and entry["eval_details"]:
                        details = entry["eval_details"]
                        if isinstance(details, str):
                            import json
                            details = json.loads(details)
                        st.json(details)
                    elif entry["eval_method"] == "human" and entry["eval_details"]:
                        details = entry["eval_details"]
                        if isinstance(details, str):
                            import json
                            details = json.loads(details)
                ...
                if entry["mlflow_run_id"]:
                    st.info(
                        f"**MLflow**\n`{entry['mlflow_run_id'][:8]}...`\n\n"
                        "[Voir dans MLflow →](http://localhost:5001)"
                    )
